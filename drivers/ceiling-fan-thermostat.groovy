/*
   ...
   It requires two hardware devices; any temperature sensor and a fan controller with the fanControl capability.
  ...
  Change Log
  2020-06-24 v1.0.0 bujvary - Changed capability to fanControl and switch to setSpeed() function
                              Added logEnable option and logsOff() function to control debug logging
  ...
  I modified the SmartThings original Virtual Thermostat code which is buggy. Known issues
...
definition(
    ...
    importURL: "https://raw.githubusercontent.com/bujvary/hubitat/master/drivers/ceiling-fan-thermostat.groovy"
)
...
def childStartPage() {
	dynamicPage(name: "childStartPage", title: "Select your devices and settings", install: true, uninstall: true) {
        ...
        section("Select the ceiling fan control hardware..."){
            input "fanDimmer", "capability.fanControl",
        ...
        section("Optional Settings (Diff Temp, Timers, Motion, etc)") {
        	...
        	description: none,
        	image: "https://raw.githubusercontent.com/bujvary/hubitat/master/images/settings250x250.png",
        	required: false,
        	page: "optionsPage"
        	)
        }
...
// VERSION
			...
			image: "https://raw.githubusercontent.com/bujvary/hubitat/master/images/3scft125x125.png",
			required: false,
			page: "aboutPage"
			)
		}
...
def installed() {
	if (logEnable) log.debug "def INSTALLED with settings: ${settings}"
	initialize()
}

def updated() {
	if (logEnable) log.debug "def UPDATED with settings: ${settings}"
	unsubscribe()
	initialize()
...
def initialize() {
	if (logEnable) log.debug "def INITIALIZE with settings: ${settings}"
...
def temperatureHandler(evt) {
	if (logEnable) log.debug "temperatureHandler called: $evt"	
    handleTemperature(evt.doubleValue)
	if (logEnable) log.debug "temperatureHandler evt.doubleValue : $evt"
}

def handleTemperature(temp) {		//
	if (logEnable) log.debug "handleTemperature called: $evt"	
	def isActive = hasBeenRecentMotion()
	if (isActive) {
		//motion detected recently
		tempCheck(temp, setpoint)
		if (logEnable) log.debug "handleTemperature ISACTIVE($isActive)"
	}
	else {
     	fanDimmer.off()
 	}
...
def motionHandler(evt) {
	if (evt.value == "active") {
		...
		if (logEnable) log.debug "motionHandler ACTIVE($isActive)"
		if (lastTemp != null) {
		...
		if (logEnable) log.debug "motionHandler INACTIVE($isActive)"
		if (isActive) {
			def lastTemp = tempSensor.currentTemperature
...
{
	if (logEnable) log.debug "TEMPCHECK#1(CT=$currentTemp,SP=$desiredTemp,FD=$fanDimmer.currentSwitch,FD_LVL=$fanDimmer.currentLevel, automode=$autoMode,FDTstring=$fanDiffTempString, FDTvalue=$fanDiffTempValue)"
	...
	if (logEnable) log.debug "TEMPCHECK#2(CT=$currentTemp,SP=$desiredTemp,FD=$fanDimmer.currentSwitch,FD_LVL=$fanDimmer.currentLevel, automode=$autoMode,FDTstring=$fanDiffTempString, FDTvalue=$fanDiffTempValue)"
	if (autoModeValue == "YES-Auto") {
    	switch (currentTemp - desiredTemp) {
        	case { it  >= HighDiff }:
        		// turn on fan high speed
                fanDimmer.setSpeed("high")
            	if (logEnable) log.debug "HI speed(CT=$currentTemp, SP=$desiredTemp, FD-LVL=$fanDimmer.currentLevel, HighDiff=$HighDiff)"
	        break  //exit switch statement 
		case { it >= MedDiff }:
            	// turn on fan medium speed
                fanDimmer.setSpeed("medium")
            	if (logEnable) log.debug "MED speed(CT=$currentTemp, SP=$desiredTemp, FD-LVL=$fanDimmer.currentLevel, MedDiff=$MedDiff)"
                break
       		case { it >= LowDiff }:
            	// turn on fan low speed
                fanDimmer.setSpeed("low")
            	if (logEnable) log.debug "LO speed immediately(CT=$currentTemp, SP=$desiredTemp, FD-LVL=$fanDimmer.currentLevel, LowDiff=$LowDiff)"
                break
		default:
            	// check to see if fan should be turned off
            	if (desiredTemp - currentTemp >= 0 ) {	//below or equal to setpoint, turn off fan, zero level
            		fanDimmer.off()
            		if (logEnable) log.debug "below SP+Diff=fan OFF (CT=$currentTemp, SP=$desiredTemp, FD-LVL=$fanDimmer.currentLevel, FD=$fanDimmer.currentSwitch,autoMode=$autoMode,)"
				} 
                if (logEnable) log.debug "autoMode YES-MANUAL? else OFF(CT=$currentTemp, SP=$desiredTemp, FD-LVL=$fanDimmer.currentLevel, FD=$fanDimmer.currentSwitch,autoMode=$autoMode,)"
        }	
	}	
}
