 ...
 *
 * Based on by Ivar Holand
 *   https://github.com/ivarho/hubitatappndevice/tree/master/Device/tuyaDevices
 *   https://raw.githubusercontent.com/ivarho/hubitatappndevice/master/Device/tuyaDevices/tuyaThreeGangSwitch.groovy
 *
 * Copyright 2026 Brian Ujvary
 *
 * Change Log:
 *   6/26/2026 v1.0 - Modified to use Hubitat Generic Component Switch for children components
 */

metadata {
	definition(
        name: "Enbrighten Smart Light Adapter",
        namespace: "iholand",
        author: "iholand",
		importUrl: "https://raw.githubusercontent.com/bujvary/hubitat/master/drivers/enbrighten-smart-light-adapter.groovy"
    ) {
		capability "Actuator"
		capability "Sensor"
...
def installed() {
	cd = addChildDevice("hubitat", "Generic Component Switch", "tuya-outlet-switch", [name: "Outlet Switch", isComponent: true])
	cd.updateDataValue("endpoint", "1")

	cd = addChildDevice("hubitat", "Generic Component Switch", "tuya-light-switch", [name: "Light Switch", isComponent: true])
...
def parse(String message) {
	...
	results.each {status_object ->
		...
		getChildDevices().each {cd->
			...
			} else if (status_object.dps[cd_endpoint] == false){
...
def on() {
	...
	if (getParent() == null) {
		send("set", ["${settings.endpoint}":true])
	} else {
		getParent().send("set", ["${settings.endpoint}":true])
	}
...
def off() {
	if (getParent() == null) {
		send("set", ["${settings.endpoint}":false])
	} else {
		getParent().send("set", ["${settings.endpoint}":false])
	}
